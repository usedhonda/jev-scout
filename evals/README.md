...
`decision-depth/` is a separate synthetic discovery slice for the depth of one proposal, including invocation-population misses, unequal error costs, failure actions, and an exact-operation no-adopt. Give the agent only that directory and its README, using the request there; keep `rubric.md` hidden. Source inspection or a syntax check is not a behavioral pass. Record what the host actually proposed before scoring against the rubric.
...
For fourth-wave discovery, assess `semantic-seams/` with the same request and hidden rubric. This fixture probes weak proxies, partial LLM decomposition, canonicalization and information insufficiency. A strong report may reject most candidates if it grounds the comparison and recognizes both negative controls.
