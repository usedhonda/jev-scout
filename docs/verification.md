...
## Third-wave control-plane discovery (2026-09-23)

The public Skill now examines decisions that govern computation and state, separately from business-item classification. Two synthetic, non-deployed workbenches cover stopping, retention, test allocation, semantic contract checks, search-frontier priority, generated-candidate evaluation, factorized actions and negative exact/real-time controls. The supplied research manuscripts remain outside this repository and Git history.

Claude Code 2.1.280 and Codex CLI 0.156.1 independently assessed both workbenches with the same read-only prompt. Neither was shown the maintainer rubric, research manuscripts or prior outputs; neither edited files or called the TypeSafe API. Both grounded a possible post-evidence stopping judgment, a human-advisory contract check and a semantic beam-search prior in code locations, retained deterministic guards, compared simpler baselines and gave falsifiable offline checks. Both identified the revision-only cache fix as a deterministic prerequisite and rejected replacing exact grammar, emergency stop or digest verification. Test impact selection and voice endpointing were treated as conditional or information-limited rather than automatic recommendations.

These reports satisfy the fixture's behavioral smoke criteria but do not establish usefulness in a production repository, measured latency/cost savings, or live Jev accuracy. Raw host transcripts are held only in ignored local storage. The public branch's source tree was reproduced without the initial unpushed local commits that used non-noreply author metadata; only noreply-authored commits are candidates for publication.
